...
def encoder(input, training, dropout):
  ...
  layer12 = conv(3, layer11, 512, strides=2, dropout=dropout, training=training)
  layer12 = conv(3, layer12, 256, strides=1, dropout=dropout, training=training)
  print('12', layer12.shape) # 12x16 32
  layer13 = conv(3, layer12, 512, strides=2, dropout=dropout, training=training)
...
def model(input, targets, training, alpha, dropout):
  ...
  train_vgg = tf.train.MomentumOptimizer(1e-6, 0.9).minimize(G_loss, var_list=[var for var in trainables if 'vgg' in var.name])
